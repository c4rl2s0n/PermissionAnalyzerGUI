...
int main(int argc, char *argv[])
{
    ...
    if(argc != 2) {
        fprintf(stderr, "use: %s input_events\n", argv[0]);
        return 1;
    }
    ...
    while (fgets(line, sizeof(line), fd_in) != NULL) {
        ...
        sscanf(line, "%lf %[^:]: %s %s %s", &timestamp_now, event_path, type, code, value);
        
        if(strncmp(open_event_path, event_path, 128) != 0){
            snprintf(open_event_path, 128, "%s", event_path);
            //fprintf(stdout, "Open Event Path: %s\n", open_event_path);
            close(fd);
            ...
            //fprintf(stdout, "Using Open Event Path: %s\n", open_event_path);
        }
